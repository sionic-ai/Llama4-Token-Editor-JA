...
def load_japanese_token_ids_from_json(json_path: str) -> List[int]:
    ...
    try:
        ...
        logging.info(f"{len(japanese_ids)} 個の日本語トークンIDをJSONから読み込みました: {json_path}")
        return japanese_ids
    ...
    try:
        with open(txt_path, "r", encoding="utf-8") as f:
            ...
            if match:
                ...
                japanese_ids = [int(id_str) for id_str in ids_str.split(',') if id_str.strip()]
                logging.info(f"{len(japanese_ids)} 個の日本語トークンIDをTXTから読み込みました: {txt_path}")
                return japanese_ids
            else:
                logging.error(f"TXTファイル内でIDリストのパターンが見つかりません: {txt_path}")
                return []
    except FileNotFoundError:
...
def main():
    parser = argparse.ArgumentParser(description="Transformersを使用してローカルでテキスト生成 (オプションで日本語トークンバイアス適用)")
    # 日本語モデルの例を使用 (または分析したモデルを指定)
    parser.add_argument('--model_id', type=str, default="stabilityai/japanese-stablelm-instruct-gamma-7b",
                        help="Hugging FaceモデルIDまたはローカルパス。")
    parser.add_argument('--prompt', type=str, default="日本の首都は",
                        help="生成のための入力プロンプト。")
    # 日本語IDファイルを指すように変更
    parser.add_argument('--japanese_ids_file', type=str, default='contains_japanese_japanese-stablelm-instruct-gamma-7b.txt', # デフォルトを具体的に
                        help="日本語トークンIDを含むファイルへのパス (TXTまたはJSON)。ファイル名はモデルによって変わります。")
    parser.add_argument('--japanese_bias', type=float, default=1.5, # 日本語バイアス
    ...
    parser.add_argument('--dtype', type=str, default="bfloat16", choices=["float16", "bfloat16", "float32"], # bf16が推奨されることが多い
    ...
    torch_dtype = torch_dtype_map.get(args.dtype, torch.bfloat16) # デフォルトをbf16に変更
    ...
    try:
        ...
        if tokenizer.pad_token is None:
             if tokenizer.eos_token is not None:
                 logging.warning("PADトークンが見つかりません。EOSトークンをPADトークンとして使用します。")
                 tokenizer.pad_token = tokenizer.eos_token
                 tokenizer.pad_token_id = tokenizer.eos_token_id # IDも設定
             else:
                 logging.error("PADトークンもEOSトークンも見つかりません。パディングが必要です。")
    ...
    # --- 2. モデルの読み込み ---
    logging.info(f"モデルを読み込み中: {args.model_id} (dtype={args.dtype}, device_map='{args.device_map}')")
    try:
        ...
        model.eval() # 評価モードに設定
         ...
         logging.error(f"インポートエラー: {e}。このモデルは特定の依存関係が必要か、trust_remote_code=Trueが不十分かもしれません。")
         return
    except Exception as e:
        logging.error(f"モデルの読み込みエラー: {e}")
        logging.info("device_mapを使用する場合は 'accelerate' がインストールされていることを確認してください (`pip install accelerate`)")
        return
    ...
    try:
       ...
       logging.warning(f"入力IDをモデルデバイス ({model.device}) に移動できませんでした: {e}。生成が失敗する可能性があります。")
    ...
    if args.japanese_bias > 0:
        ...
        if not os.path.exists(ids_file):
             logging.warning(f"指定されたIDファイルが見つかりません: {ids_file}. デフォルトのファイル名を試します。")
             # モデル名からデフォルトファイル名を推測
             ...
             if os.path.exists(default_txt_file):
                 ...
                 if os.path.exists(default_json_file):
                     ...
                     logging.error("IDファイルが見つかりません。バイアスは適用されません。")
        ...
        if os.path.exists(ids_file):
            if ids_file.endswith(".txt"):
                ...
                logging.warning(f"不明なIDファイル形式: {ids_file}。TXTとJSONローダーを試します。")
                japanese_ids = load_japanese_token_ids_from_txt(ids_file) or load_japanese_token_ids_from_json(ids_file)

        if japanese_ids:
            token_bias_dict = {token_id: args.japanese_bias for token_id in japanese_ids}
            token_bias_processor = TokenBiasLogitsProcessor(token_bias_dict)
            logits_processor = LogitsProcessorList([token_bias_processor])
            logging.info(f"{len(japanese_ids)} 個のバイアス付きIDを持つLogitsProcessorを作成しました。")
        elif os.path.exists(ids_file): # ファイルは存在するがIDが読み込めなかった場合
            logging.warning(f"ファイル {ids_file} から日本語IDを読み込めませんでした。バイアスなしで続行します。")
    ...
    if max_new_tokens <= 0 :
        logging.warning(f"max_length ({args.max_length}) がプロンプト長 ({input_ids.shape[1]}) 以下です。max_new_tokensを50に設定します。")
        max_new_tokens = 50
    ...
    if logits_processor:
        ...
        try:
            ...
            # 生成された部分のみをデコード
            generated_ids_with_processor = output_with_processor[0][input_ids.shape[1]:]
            generated_text_with_processor = tokenizer.decode(generated_ids_with_processor, skip_special_tokens=True)
            print("出力 (バイアスあり):")
    ...
    try:
        ...
        generated_text_without_processor = tokenizer.decode(generated_ids_no_processor, skip_special_tokens=True)
        print("出力 (バイアスなし):")
