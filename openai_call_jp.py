...
def load_japanese_token_ids_from_json(json_path: str) -> List[int]:
    ...
    try:
        ...
        japanese_ids = token_ids_data.get('contains_japanese', []) # 'contains_japanese' を使用
...
def main():
    parser = argparse.ArgumentParser(description="OpenAI APIを使用してテキスト生成 (オプションで日本語logitバイアス適用)")
    # OpenRouter等で利用可能な日本語対応モデル名を指定する必要がある
    parser.add_argument('--model_name', type=str, default="mistralai/mixtral-8x7b-instruct", # 例: Mixtralは多言語対応
    ...
    parser.add_argument('--prompt', type=str, default="日本の有名な食べ物といえば？",
                        help="生成のための入力プロンプト。")
    # 日本語IDファイルを指定
    parser.add_argument('--japanese_ids_file', type=str, default='contains_japanese_mixtral-8x7b-instruct.txt', # ファイル名はモデル依存
                        help="日本語トークンIDを含むファイルへのパス (TXTまたはJSON)。")
    parser.add_argument('--japanese_bias', type=float, default=5.0, # 日本語バイアス
    ...
    if not args.api_key:
        logging.error("APIキーが指定されていません。OPENROUTER_API_KEY環境変数を設定するか、--api_keyを使用してください。")
        return
    ...
    if args.japanese_bias != 0:
        ...
        if not os.path.exists(ids_file):
             ...
             model_name_part = args.model_name.split('/')[-1].replace('-', '_') # APIモデル名を使用
        ...
        if japanese_ids:
             ...
             if clamped_bias != args.japanese_bias:
                 logging.warning(f"API互換性のため、バイアス値を {args.japanese_bias} から {clamped_bias} に制限しました。")
             # キーは文字列である必要がある
             logit_bias_dict = {str(token_id): clamped_bias for token_id in japanese_ids}
             logging.info(f"{len(logit_bias_dict)} 件のエントリを持つlogitバイアス辞書を準備しました (バイアス値: {clamped_bias})")
